...
Route::get('/register/verify/{confirmationCode}', 'Auth\RegisterController@confirm')
	 ->name('confirm-email');
Route::any('/register/verify/resend/{user}', 'Auth\RegisterController@resend')
	 ->name('confirm-email-resend');
	 ...
	 ->group(function () {
	   ...
	   Route::prefix('application')
			...
			->group(function () {
			  ...
			  Route::get('/{jobListing}/create', 'JobListingApplicationController@create')
				   ->name('create');
			  Route::post('/{jobListing}/create', 'JobListingApplicationController@store')
				   ->name('store');

			  Route::post('/{application}/update', 'JobListingApplicationController@update')
				   ->name('update');
			  Route::get('/{application}/edit', 'JobListingApplicationController@show')
				   ->name('show');
			});

	   Route::any('/application/{application}/redirect', 'JobListingApplicationController@permalink')
			->name('application.permalink');
	   Route::get('/application/{application}', 'JobListingController@showApplication')
...
Route::prefix('account')
	 ...
	 ->group(function () {
	   ...
	   Route::get('/notification-preferences', 'UserController@editNotificationPreferences')
			->name('manage.notification-preferences');
	   Route::post('/notification-preferences', 'UserController@updateNotificationPreferences')
			->name('manage.notification-preferences');

	   Route::get('/private-messages', 'PrivateMessageController@index')
			->name('private-message.index');
	   Route::post('/private-messages/send', 'PrivateMessageController@store')
			->name('private-message.store');
...
Route::prefix('company')
	 ...
	 ->group(function () {
	   ...
	   Route::post('/applications', 'CompanyController@getApplications')
			->name('application.index.get');
...
Route::prefix('cv')
	 ->name('cv.')
	 ->group(function () {
	   Route::resource('education', 'Cv\EducationController', [
		 'only' => [
		   'store', 'update', 'destroy',
		 ]]);

	   Route::resource('workExperience', 'Cv\WorkExperienceController', [
		 'only' => [
		   'store', 'update', 'destroy',
		 ]]);
	   ...
	   Route::put('preferences', 'Cv\PreferencesController@update')->name('preferences.update');
...
Route::post('/accept-invite/{invite}', 'CompanyUserInviteController@accept')
	 ->name('accept-invite.accept');
...
Route::prefix('notifications')
	 ...
	 ->group(function () {
	   ...
	   Route::post('/get', 'NotificationController@get')->name('get');
	   Route::any('/all/mark/read', 'NotificationController@markAllAsRead')
			->name('mark-all-as-read');
	   Route::any('/all/mark/unread', 'NotificationController@markAllAsUnread')
			->name('mark-all-as-unread');
	   Route::any('/all/delete/all', 'NotificationController@deleteAll')->name('delete-all');
	   Route::any('/all/delete/read', 'NotificationController@deleteAllRead')
			->name('delete-all-read');

	   Route::any('/click-through/{notification}/{prop?}', 'NotificationController@clickThrough')
			->name('click-through');

	   Route::any('/{notification}/mark-as-read', 'NotificationController@markAsRead')
			->name('mark-as-read');
	   Route::any('/{notification}/mark-as-unread', 'NotificationController@markAsUnread')
			->name('mark-as-unread');
